  ...
  const load3DObject = useCallback(async (file: File) => {
    ...
    try {
      ...
      switch (fileExtension) {
        ...
        case 'igs':
        case 'stl': {
          // ✅ STEP/IGES/STL dosyaları için yüzey bazlı segmentation kullan
          debug.log('[3D Viewer] CAD file detected, using face segmentation:', fileExtension);

          // CADLoader kullanarak yüzey bazlı yükleme yap (CDN versiyonu)
          try {
            const { loadCADFileWithCDN } = await import('../cad-viewer/cadLoaderCDN');

            // Loader'ı promise olarak kullan
            const cadMeshes = await new Promise<THREE.Mesh[]>((resolve, reject) => {
              const onLoad = (meshes: THREE.Mesh[]) => {
                resolve(meshes);
              };
              ...
              // CDN helper fonksiyonu kullanarak yüzey bazlı yükleme yap
              loadCADFileWithCDN(file, sceneRef.current!, onLoad, onError);
            });

            // Yüklenen mesh'leri group'a ekle
            if (cadMeshes.length > 0) {
              object = new THREE.Group();
              cadMeshes.forEach((mesh, index) => {
                // Dosya adı ve index'e dayalı deterministik handle oluştur
                // Bu sayede modal kapatılıp açıldığında aynı mesh'ler aynı handle'a sahip olur
                const fileBaseName = file.name.replace(/\.[^/.]+$/, ""); // Uzantıyı kaldır
                mesh.userData = {
                  ...mesh.userData,
                  type: 'cadFace',
                  handle: `cad_face_${fileBaseName}_${index}`,
                  fileName: file.name,
                  selectable: true,
              ...
              debug.log(`[3D Viewer] CAD file loaded with ${cadMeshes.length} faces`);
            }
          } catch (cadError) {
            debug.error('[3D Viewer] CAD file loading error:', cadError);
            throw new Error(`Error loading CAD file: ${cadError instanceof Error ? cadError.message : 'Unknown CAD error'}`);
          }
          break;
        }
        case 'ply': {
