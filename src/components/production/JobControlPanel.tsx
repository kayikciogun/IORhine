  ...
  return (
    <div className="space-y-1.5">
      <div className="flex flex-wrap gap-2" role="group" aria-label="Job kontrol">
        <Button size="sm" className="h-8 text-xs gap-1" onClick={onStart} disabled={disabled || !canStart} aria-label="Job başlat">
          <Play className="w-3.5 h-3.5" /> Başlat
        </Button>
        <Button size="sm" variant="secondary" className="h-8 text-xs gap-1" onClick={onPause} disabled={disabled || !canPause} aria-label="Job duraklat">
          <Pause className="w-3.5 h-3.5" /> Duraklat
        </Button>
        <Button size="sm" variant="secondary" className="h-8 text-xs gap-1" onClick={onResume} disabled={disabled || !canResume} aria-label="Job devam ettir">
          <RotateCcw className="w-3.5 h-3.5" /> Devam
        </Button>
        <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={onStop} disabled={disabled} aria-label="Job durdur">
          <Square className="w-3.5 h-3.5" /> Durdur
        </Button>
        <Button size="sm" variant="destructive" className="h-8 text-xs gap-1" onClick={onEstop} disabled={disabled} aria-label="Acil durdur">
