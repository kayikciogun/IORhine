...
export default function PickPlaceHome() {
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden text-foreground">
      ...
      <div className="w-[420px] lg:w-[460px] shrink-0 bg-card border-l border-border h-full flex flex-col shadow-xl z-20">
        <div className="p-3.5 border-b border-border bg-muted/30 flex justify-between items-center gap-3">
          <h1 className="text-base font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent tracking-tight">
            IO-CAM Pick & Place
          </h1>
          <span
            className="inline-flex items-center gap-1.5 rounded-md border border-dashed border-border px-2.5 py-1 text-[10px] text-muted-foreground"
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3 custom-scrollbar">
          {/* Taş Tipi Yönetimi */}
          <div className="rounded-lg border border-border bg-background p-3.5 shadow-sm flex flex-col shrink-0">
            <StoneTypePanel />
          </div>
          ...
          {/* CSV dışa aktarma */}
          <div className="flex-1 flex flex-col rounded-lg border border-border bg-background p-3.5 shadow-sm min-h-0 overflow-hidden">
            <ExportPanel />
          </div>
        </div>
      </div>
